            ...
            <div id="iam.roles" v-bind:key="roleName" v-for="roleName in roleNames">
                <div v-bind:id="'iam.roles' + '.' + getPrincipalMetadata(roleName, 'Role')['id'] + '.' + 'details'">
                    ...
                    <div class="list-group-item">
                        ...
                        <div class="list-group-item-text item-margin">Creation date: {{ getPrincipalMetadata(roleName,
                            'Role')['create_date'] }}
                        </div>
                        <div class="list-group-item-text item-margin">ARN: {{ getPrincipalMetadata(roleName,
                            'Role')['id'] }}
                        </div>
                    </div>
                    <!--Group item: Risks-->
                            ...
                            <div v-bind:key="riskName" v-for="riskName in riskNames">
                                <div class="row">
                                    <b-list-group-item :action="true"
                                        ...
                                        <b-badge variant="info">{{ getRiskAssociatedWithPrincipal(roleName, "Role",
                                            riskName).length }}
                                            <span class="sr-only">{{addSpacesInPascalCaseString(riskName)}} risks</span>
                                        </b-badge>
                                        <br>
                                    </b-list-group-item>
                                </div>
